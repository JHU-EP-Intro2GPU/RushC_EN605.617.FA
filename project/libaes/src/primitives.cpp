...
/// The Rijndael S-box as defined in AES.
...
namespace aes
{    
    uint8_t rotate_left(uint8_t byte, uint8_t amount) 
    {
        return (byte << amount) | (byte >> (BITS_PER_BYTE - amount));
    }
    
    uint32_t rotate_left(uint32_t word, uint8_t amount) 
    {
        uint32_t rotated = 0;

        auto word_bytes = reinterpret_cast<uint8_t*>(&word);
        auto rotated_bytes = reinterpret_cast<uint8_t*>(&rotated);

        for (int i = 0; i < sizeof(uint32_t); i++)
        {
            rotated_bytes[i] = word_bytes[(i + amount) % sizeof(uint32_t)];
        }
    ...
    uint32_t substitute(uint32_t word) 
    {
        ...
        substitute(reinterpret_cast<uint8_t*>(&word),
                   sizeof(word),
                   reinterpret_cast<uint8_t*>(&substituted));
        return substituted;
    }
    
    void substitute(const uint8_t bytes[], size_t num_bytes, uint8_t out[]) 
    {
        std::transform(bytes, &bytes[num_bytes], out, 
                       static_cast<uint8_t (*)(uint8_t)>(substitute));
    }
}